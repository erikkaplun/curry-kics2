...
In this section we evaluate our implementation by a set of benchmarks.
...
are \code{last} (compute the last element of a list),
...
and
\code{palindrom} (check whether a list is a palindrom).

In Section \ref{sec:Unification} we discussed that equational
...
we compare the results of equational constraints to results
...
Since we have added only additional constructors and rules
for these constructors, our implementation does not cause
additional work if these features are not used, i.e.,
our implementation does not sacrifice the high efficiency
of the kernel implementation as described in
\cite{BrasselFischerHanusReck11}.
However, if these features are used, they usually lead
to a more efficient execution, as shown by our benchmarks.
...
\todo{Future work: adding other constraint systems}
