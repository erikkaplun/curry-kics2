...
One could also use compile into logic languages like Prolog
...
There are also more recent approaches
\cite{BrasselHuch07,BrasselHuch09}
where functional logic programs are compiled
...
In KICS2, logic variables are replaced by generators,
...
is equivalent, i.e., yields the same values \cite{AntoyHanus06ICLP}.
Although this implementation technique
outperforms other existing implementations of Curry
...
as tree traversals. Thus, KiCS2 supports a primitive
\begin{curry}
...
as tree traversals (see \cite{HanusPeemoellerReck12ATPS}
...
all elements of the list are (unnecessarily) evaluated. On the other
hand, lazy unification causes some unnecessary overhead when all elements are
evaluated anyway.
