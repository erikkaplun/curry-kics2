...
For instance, functional patterns support a straightforward
...
\end{haskell}
This operation ignores failures and prints values that are not a \code{Choice}.
...
stores the decision made by the constraint.
In this case, the constrained value is evaluated
before the constraint is removed to allow backtracking.
...
In this section we evaluate our implementation by some benchmarks.
They were executed on a Linux machine running Debian 5.0.7 with
an Intel Core 2 Duo (3.0GHz) processor.
KiCS2 has been executed with
the Glasgow Haskell Compiler (GHC 7.0.4, option -O2).
...
\hline
\code{last (map (inc 0) [1..100000])}   & 134.23    & 0.83      & 0.26        \\
% \code{grep}                             & 1.06      & 0.19      &             \\
\code{simplify}                         & 8.50      & 7.15      & 7.34        \\
\code{varInExp}                         & 11.72     & 1.50      & 0.14        \\
% \code{varInExp2}                        & 423.01    & 10.50     & 1.04        \\
% \code{varInExp3}                        & 399.68    & 16.70     & 11.28       \\
\code{fromPeano (half (toPeano 10000))} & 32.92     & 6.35      & 11.88       \\
\code{palindrome}                       & 28.59     & 15.58     & 23.21       \\
% \code{horseman}                         & 3.58      & 3.13      &             \\
\hline
\end{tabular}
\caption{Benchmarks: comparing different equation solving methods}
...
we compare in Fig.~\ref{fig:search-strategies}
...
by \code{==} is in most cases much slower than our presented approach
...
\hline
\code{last (map (inc 0) [1..100000])}   & 0.83  & 4.25   & 0.14 \\
% \code{grep}                             & 0.19  & 2.88   & 0.14 \\
\code{simplify}                         & 7.15  & 0.15   & 0.00 \\
\code{varInExp}                         & 1.50  & 3.89   & 0.30 \\
% \code{varInExp2}                        & 10.50 & 3.48   & stckOflw \\
% \code{varInExp3}                        & 16.70 & 19.22  & 1.18 \\
\code{fromPeano (half (toPeano 10000))} & 6.35  & 108.88 & 3.22 \\
\code{palindrome}                       & 15.58 & 32.56  & 1.07 \\
% \code{horseman}                         & 3.13  & 8.70   & 0.42 \\
\hline
\end{tabular}
\caption{Benchmarks: equational constraints in different Curry implementations}
 \label{fig:unification}
\end{figure}

In contrast to the Curry implementations PAKCS and MCC, our approach is based
...
show that it can compete with or even outperform these implementations.
In these benchmarks only equational constraints are used.
The results show that the implementation of unification
of MCC performs best. However, in most cases
our implementation outperforms the Prolog-based PAKCS implementation,
except for some examples where further investigations
...
\hline
\code{last (map (inc 0) [1..100000])}   & 0.26  & 3.97     \\
\code{simplify}                         & 7.34  & 0.27     \\
\code{varInExp}                         & 1.40  & 8.88     \\
% \code{varInExp2}                         & 1.04  & 0.08    \\
% \code{varInExp3}                         & 11.28 & 16.62   \\
\code{fromPeano (half (toPeano 10000))} & 11.88 & $\infty$ \\
\code{palindrome}                       & 23.21 & $\infty$ \\
\hline
\end{tabular}
\caption{Benchmarks: comparing different implementations of functional patterns}
...
of more than 30 minutes). \todo{extend}
...
and functional patterns in a purely functional implementation
...
Since we have added only new constructors and rules
for them in our implementation, it does not introduce
additional work if these features are not used, i.e.,
our implementation does not sacrifice the high efficiency
of the kernel implementation as shown in
\cite{BrasselFischerHanusReck11}.
However, if these features are used, they usually lead
to a more efficient execution, as demonstrated by our benchmarks.
...
further constraint structures, like real arithmetic
or finite domain constraints, to our implementation.
