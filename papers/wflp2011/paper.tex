...
nondeterministic result values of an expression as a data structure.
This enables the application of various search strategies
to extract the different values from the search space.
In addition to the increased functionality of our system,
we demonstrate by a several benchmarks that our implementation
...
directly map operations on numbers directly into the Haskell primitives)
where the GHC can apply particular optimizations.
...
``Reverse'' reverses a list in linear time using higher-order functions
like \code{foldl} and \code{flip} applied to a list with one million
elements.
...
is shown as ``KiCS2HO'' and demonstrates its usefulness so that
it is always used in the subsequent benchmarks.

\begin{figure}
\centering
\begin{tabular}{|@{~~}l@{~~}|*{3}{@{~~}r@{~~}|}}
\hline
System  & PermSort & PermSortPeano &  Last \\\hline
KiCS2   &     3.23 &          1.26 &  0.26 \\
PAKCS   &    26.08 &         21.75 &  2.58 \\
MCC     &     1.23 &          1.28 &  0.08 \\
\hline
\end{tabular}
...
for Peano numbers and user-defined lists with 14 elements)
and computed the last element \code{x}
...
Although the results shows that our implementation cannot compete
with MCC, it is superior to PAKCS which exploits Prolog features
...
\begin{curry}
  goal1 = [primes!!803, primes!!802, primes!!801, primes!!800]
  goal2 = psort [6173,6163,6151,6143]
  goal3 = psort [primes!!803, primes!!802, primes!!801, primes!!800]
\end{curry}
...
\begin{tabular}{|@{~~}l@{~~}|*{3}{@{~~}r@{~~}|}}
\hline
System  &   goal1  &   goal2 & goal3 \\\hline
KiCS2   &    0.32  &   0.00  &  0.32 \\
PAKCS   &    8.88  &   0.02  & 91.68 \\
MCC     &    0.20  &   0.00  &  1.94 \\
\hline
\end{tabular}
...
Due to lack of space, we cannot discuss all details of the implementation.
...
\code{printValsDFS}  &    3.24  &         1.26  & $\infty$ \\
depth-first search   &    3.63  &         1.40  & $\infty$ \\
breadth-first search &   18.30  &         6.85  &    11.52 \\
iterative deepening  &    5.94  &         1.20  &     3.22 \\
%parallel (1 proc.)   &    5.84  &         2.09  & $\infty$ \\
%parallel (2 proc.)   &    3.46  &         1.25  & $\infty$ \\
\hline
\end{tabular}
...
Fig.~\ref{fig:search-strategies} contains corresponding
benchmarks where ``PermSort'' and ``PermSortPeano'' are the programs
...
is negligible so that this more flexible approach is the default one.
...
into tree structure of the form
\begin{curry}
 data SearchTree a = Value a | Fail | Or (SearchTree a) (SearchTree a)
\end{curry}
so that the programmer can define its own search strategy or to
...
to put conditions one computed results.
...
of this extension.
However, it should be noted that the example
``Last'' of Fig.~\ref{fig:bench-nondet}
...
Apart from the parser, where we reused an existing parser
...
programs, which is an interesting topic for future work.
