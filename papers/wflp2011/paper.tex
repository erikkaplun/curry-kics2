...
non-deterministic results as values in the data types corresponding
to the results.
...
Uniform programs have been also considered in
...
our notion of uniform program is more restrictive since we
...
\item Each choice introduced in a computation gets its own unique
identifier.
\end{enumerate}
...
\code{leftSupply} and \code{rightSupply} splits this set
...
the other operations must be supplied with their individual disjoint
...
\end{haskell}
Consequently, an implementation of \code{IDSupply} requires
...
where the operation \code{unsafeInterleaveIO} is used to construct
the tree on demand:
\begin{haskell}
...
We have already discussed in the introduction the various
...
Due to these experiences, we used in our new compiler the
