------------------------------------------------------------------------------
--- A pretty printer for AbstractClassCurry programs.
---
--- This library defines a function "showProg" that shows
--- an AbstractClassCurry program in standard Curry syntax.
---
...
showExports types funcs = 
  ...
  where
    ...
    isPublicType (CInstance _ _ _ _) = False
    ...
    getTypeName (CInstance (_,name) _ _ _) = name
    ...
    allPublicCons (CInstance _ _ _ _) = False
...
showTypeDecl (CInstance (_,name) texp ctxts rules)
...
showContext :: [CContext] -> String
showContext [] = ""
showContext [ctxt] = showClass ctxt ++ " => "
showContext cs@(_:_:_) =
  "(" ++ concat (intersperse "," (map showClass cs)) ++ ") => "
   
showClass (CContext qn tvars) = showTypeExpr False (CTCons qn (map CTVar tvars))
