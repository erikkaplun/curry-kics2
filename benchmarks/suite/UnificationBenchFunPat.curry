module UnificationBenchFunPat where
...
lengthUpToRepeatL (p++[r]++_) = if nub p == p && elem r p 
                                 then length p + 1
                                 else failed
...
goal3L = lengthUpToRepeatL ([1..50] ++ [1] ++ [51 ..])
...
goal5L  = fromPeano (halfL  (toPeano 10000))
...
varInExpL :: Exp -> VarName
varInExpL (replace _ _ (Var v))= v
...
goal6L  = varInExpL  (genExpWithVar 10000)
...
simplifyL :: Exp -> Exp
simplifyL (replace c p (evalTo x)) = replace c p x
...
goal7L  = expSize (simplifyL  (genExpWithMult1 2000))
...
paliL :: [a] -> Success
paliL (xs ++ reverse xs) = success
paliL (xs ++ _ : reverse xs) = success
