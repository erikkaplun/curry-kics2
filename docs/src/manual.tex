...
Version of September 29, 2011\\[6ex]
\Large
...
with an initial depth bound of $n$. The depth-bound is
duplicated after each iteration.
If $n$ is missing, the default value $n=100$ is used.

\item[\fbox{\code{par $n$}}]\pindex{par}
Similarly to \code{dfs} but use a parallel search strategy
with $n$ threads.
If $n$ is missing, the system chooses an appropriate number
...
Define additional run-time options passed to the Glasgow Haskell Compiler
(by \ccode{+RTS $o$ -RTS}).
...
\newcommand{\currymodule}[2]{\subsubsection{Library #1}\label{Library:#1} #2}
...
\newcommand{\currycons}[3]{\item {\tt #1~::~#2}\pindex{#1}\par #3}
