...
\begin{exampleblock}{Example \hspace{4cm}\only<3>{(with \alert{equational constraint}~ \texttt{=:=})}\only<4>{(with \alert{functional pattern})}}
\begin{curry}
...
Main call: \code{initSupply >>= \bs{}s -> printValsDFS (main s)}\\
Result:~~~~ \code{False}\\
\hspace{1.62cm}\code{False}
\end{exampleblock}
