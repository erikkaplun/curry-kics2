...
\begin{itemize}
 \item \code{ID}s are provided by an \code{IDSupply} (infite set of \code{ID}s)
...
\node (c) at (0,2) {$\code{?}_{1}$};
\node (d) at (2,2) {$\code{?}_{1}$};
\node (e) at (1,3) {$\code{?}_{1}$};
\draw (e) -- (c);
\draw (e) -- (d);
...
\end{center}
\todo{include KiCS, remove+, update graphics}
\end{frame}
...
\end{center}
\todo{include KiCS, Prolog systems, update graphics}
\end{frame}
