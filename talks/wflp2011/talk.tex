...
\item Benefit from mature Haskell compiler (GHC) and its future improvements
\end{itemize}
\end{block}
\pause
\begin{block}{Goals}
...
\item Both occurrences of {\tt x} are substituted with the same value 
...
\item Substitutions of {\tt x} may evaluate to different values 
...
\item Only consistent decisions for different occurrences of the same Choice
\item Functions that may introduce non-determinism
      are supplied with an additional parameter to obtain fresh \verb!ID!s
\end{itemize}
\pause
...
aBool :: \alert{ID ->} Bool
aBool \alert{i} = Choice \alert{i} True False
