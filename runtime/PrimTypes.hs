...
instance Coverable BinInt where
  ...
  cover (Fail_BinInt cd info) = Fail_BinInt (cd + 1) info
...
instance Coverable Nat where
  ...
  cover (Fail_Nat cd info) = Fail_Nat (cd + 1) info
...
instance Coverable (Func t0 t1) where
  ...
  cover (Fail_Func cd info) = Fail_Func (cd + 1) info
...
instance Coverable (C_IO t0) where
  ...
  cover (Fail_C_IO cd info) = Fail_C_IO (cd + 1) info
...
instance Coverable (PrimData a) where
  ...
  cover (Fail_PrimData cd info)  = Fail_PrimData (cd + 1) info
