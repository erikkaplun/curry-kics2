...
instance Curry a => Curry (C_Global a) where
  (=?=) = error "(=?=) is undefined for Globals"
  (<?=) = error "(<?=) is undefined for Globals"


external_d_C_global :: Curry a => a -> C_GlobalSpec -> C_Global a
...
external_d_C_prim_readGlobal :: Curry a => C_Global a -> C_IO a
...
external_d_C_prim_writeGlobal :: Curry a => C_Global a -> a -> C_IO OP_Unit
external_d_C_prim_writeGlobal (C_Global_Temp ref) val =
  fromIO (writeIORef ref val >> return OP_Unit)
